        ...
        <div className="flex flex-wrap gap-2 border-b border-[var(--border-warm)] pb-3">
          {routines.map((r) => (
            <button
              ...
              className={`rounded-full px-3.5 py-1.5 text-xs font-bold transition-all border ${selectedRoutine?.id === r.id ? "bg-[var(--primary)] text-[#f5f8ff] border-[var(--primary)]" : "bg-[var(--surface)] text-[var(--color-text-muted)] border-[var(--border-warm)] hover:bg-[var(--surface-muted)]"}`}
            >
              {getShortName(r)}
            </button>
          ))}
