    ...
    <div className="mx-auto flex min-h-screen w-full max-w-md flex-col px-4 pb-[calc(8rem+env(safe-area-inset-bottom))] pt-5">
      <header className="mb-5 flex items-start justify-between gap-3">
        <Link href="/" className="min-w-0">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-[#17201d]">{title || thaiAppName}</h1>
          {subtitle ? <p className="mt-1 text-sm leading-6 text-slate-600">{subtitle}</p> : null}
        </Link>
        <Link href="/settings" aria-label="ตั้งค่า" className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white/80 shadow-sm text-slate-500 hover:text-slate-700">
