...
test("Today page shows ภาพรวมวันนี้ and วันนี้ควรทำอะไร headings in correct order", async ({ page }) => {
  ...
  // วันนี้ทำอะไรดี? is the internal heading of the hero card (second card)
  await expect(page.getByText("วันนี้ทำอะไรดี?", { exact: true })).toBeVisible();
  // ภาพรวมวันนี้ must appear before วันนี้ทำอะไรดี? in the DOM
  const overviewPos = await page.getByText("ภาพรวมวันนี้", { exact: true }).first().boundingBox();
  const heroPos = await page.getByText("วันนี้ทำอะไรดี?", { exact: true }).boundingBox();
  if (overviewPos && heroPos) {
    expect(overviewPos.y).toBeLessThan(heroPos.y);
  }
  // "สรุปท้ายวัน" section still present (its own card heading covers the outer "สรุป" label,
  // which was intentionally removed as a redundant duplicate — see EndOfDaySummaryCard)
  await expect(page.locator("#end-of-day-summary").getByText("สรุปท้ายวัน").first()).toBeVisible();
});
