...
test("Today: 4 recovery factor bars visible without expanding accordion", async ({ page }) => {
  const state = await installMockBackend(page);
  ...
  // All 4 titles visible without any click
...
test("Today: hero card shows a reason line referencing recovery summary", async ({ page }) => {
  ...
  // Hero shows reason line containing Load
  await expect(page.getByText(/Load (สูงมาก|สูง|ปานกลาง)/)).toBeVisible();
});
